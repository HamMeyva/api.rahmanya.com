...
@section('title', 'Sanatçılar')
@section('breadcrumb')
<x-admin.breadcrumb data="Sanatçılar" />
@endsection
@section('master')
<!--begin::Card-->
<div class="card">
...
<script>
    $(document).ready(function() {
        let dataTable = $("#dataTable").DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "{{ route('admin.artists.data-table') }}",
                "type": "POST",
                "data": function(d) {
                    d._token = "{{ csrf_token() }}"
                },
        ...
        const modal = $('#primaryModal'),
            form = $('#primaryForm'),
            storeUrl = "{{ route('admin.artists.store') }}",
            updateUrl = "{{ route('admin.artists.update') }}";
        ...
        $(document).on('click', '.editBtn', function() {
            ...
            $.ajax({
                type: 'GET',
                url: `{{ route('admin.artists.show') }}/${id}`,
                dataType: 'json',
        ...
        $(document).on('click', '.deleteBtn', function() {
                Swal.fire({
                    ...
                    if (r.isConfirmed) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: `{{ route('admin.artists.delete') }}/${id}`,
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
                            dataType: 'json',
